...
export default function Rank() {
  ...
  const [AssetsRecord, setAssetsRecord] = useState<any>([]);
  ...
  const getInitData = async () => {
    ...
    try {
      abi_data = await Contracts.example?.viewFoundsRecord(
    ...
    abi_data["0"]?.map((item: any, index: any) => {
      Arr.push({
        flow: item,
        amount: abi_data["1"][index],
        coinName: abi_data["2"][index],
        type: abi_data["3"][index],
        time: abi_data["4"][index],
      });
    });
    setAssetsRecord(Arr.reverse() || []);
  };
        ...
        <div className="table">
          ...
          {AssetsRecord?.map((item: any, index: any) => (
            <div className="table_content items" key={index}>
              <div className="item">
                {dateFormat("HH:MM mm/dd", new Date(item?.time * 1000))}
              </div>
              <div className="item">
                {Number(item?.flow) === 1 ? "+" : "-"}
